...
struct Args {
    ...
    /// Path to the config file to use. Defaults to ./.asdfler.yml
    #[arg(short, long, default_value_t = String::from("./.asdfler.yml"))]
    filepath: String,
}
...
fn run_plugin_add(plugin_name: &str) -> anyhow::Result<String> {
    let output = process::Command::new(ASDF)
...
fn install_version(plugin_name: &str, version: &String) -> anyhow::Result<()> {
    ...
    match run_command(process::Command::new(ASDF).args(["install", plugin_name, version])) {
        Ok(_) => {
...
fn set_default_version(plugin_name: &str, version: &String) {
    let output = run_command(process::Command::new(ASDF).args(["global", plugin_name, version]));
...
fn run_command(cmd: &mut process::Command) -> anyhow::Result<Output> {
